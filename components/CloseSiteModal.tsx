...
const CloseSiteModal: React.FC<CloseSiteModalProps> = ({ isOpen, site, onClose, onConfirm }) => {
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[95vw] sm:max-w-lg bg-white rounded-[2.5rem] shadow-2xl z-[210] animate-in zoom-in-95 fade-in duration-300 p-8 m-4">
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-sm text-slate-500 font-medium">
            Cette action va archiver définitivement ce chantier
          </p>
        </div>
        ...
        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 mb-6">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              <p className="text-xs font-bold text-slate-800 mb-2">Chantier concerné :</p>
              <p className="text-sm font-black text-slate-900">{site.name}</p>
              <p className="text-[10px] text-slate-400 font-medium mt-1">{site.address}</p>
            </div>
          </div>
          ...
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 mt-3">
            <p className="text-[10px] font-bold text-amber-700 flex items-start gap-2">
              ...
              <span>Une fois clôturé, ce chantier sera déplacé dans l'historique du client et ne sera plus modifiable.</span>
            </p>
          </div>
        </div>
